...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self,
                 path,   # my_data.data中的train/val .txt文件路径
                 # 这里设置的是预处理后输出的图片尺寸
        ...
        try:
            path = str(Path(path))  # train.txt Or val.txt
            # parent = str(Path(path).parent) + os.sep
            if os.path.isfile(path):  # file
                # 读取对应train/val.txt文件，读取每一行的图片路径信息
        ...
        # labels: [class, x, y, w, h] 其中的xywh都为相对值
        ...
        for i, file in enumerate(pbar):
            ...
            if l.shape[0]:
                ...
                # 把每个图片中的目标信息裁剪下来，存储为单独的图片，可以用来进行分类
    ...
    def __getitem__(self, index):
        ...
        if self.mosaic:
            if self.stop_mosaic is False:
                ...
                img, (h0, w0), (h, w) = load_image(self, index)  # 把图片最长边reshape到self.img_size保持宽高比不变
