...
def create_model(num_joints, pretrain_weights_path=""):
    model = Lite_HRNet_SimCC(num_joints=num_joints, input_size=args.resize_hw,
    ...
    if os.path.exists(pretrain_weights_path):
        ...
        assert "[ERROR] Need pretrain weights file!"
...
def draw_batch_keypoints(outputs, targets, save_img: bool):
    batch_keypoints = outputs[0]  # [batch_size, nkpt, 2]
    num_keypoints = batch_keypoints.shape[1]
    batch_scores = outputs[1]      # [batch_size, nkpt, 1]
    batch_size = len(targets)
    for i in range(batch_size):
        keypoints = batch_keypoints[i].astype(np.int64).tolist()
        scores = batch_scores[i][0]
        image_path = targets[i]["image_path"]
        image = cv2.imread(image_path)
        for j in range(num_keypoints):
            cv2.circle(image, keypoints[j], 2, (0, 255, 0), 8)
        if save_img == True:
            cv2.imwrite(os.path.join("outputs", "val_results","val_" + os.path.basename(image_path)),
                        image)
...
def main(args):
    ...
    # create model
    model = create_model(num_joints=21, pretrain_weights_path=args.weights_path)
    ...
    with torch.no_grad():
        for images, targets in tqdm(val_dataset_loader, desc="validation..."):
            ...
            # inference
            outputs_x, outputs_y = model(images)
            # decode keypoint
            reverse_trans = [t["reverse_trans"] for t in targets]
            outputs = transforms.get_final_preds(outputs_x, outputs_y, args.simcc_split_ratio, reverse_trans)
            if args.save_result:
                draw_batch_keypoints(outputs, targets, True)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=__doc__)
    # 是否保存检测结果图片
    parser.add_argument('--save-result', type=bool, default=False, help="whether save validation results")
    # simcc split ratio::198 use 2, 64 use 3
    parser.add_argument('--simcc-split-ratio', type=float, default=2.0, help="simcc split ratio")
    # 使用设备类型
    parser.add_argument('--device', default='cuda:0', help='device')

    parser.add_argument('--resize-hw', type=list, default=[128, 128], help="resize for predict")
    # 是否开启图像翻转
    ...
    # 训练好的权重文件
    parser.add_argument('--weights-path', default='.pth', type=str, help='training weights')

    # batch size
    parser.add_argument('--batch-size', default=32, type=int, metavar='N',
                        help='batch size when validation.')
    # 类别索引和类别名称对应关系
    parser.add_argument('--label-json-path', type=str, default="hand_21_keypoints.json")
    # 原项目提供的验证集target检测信息，如果要使用GT信息，直接将该参数置为None
    parser.add_argument('--target-det', type=str, default=None)
