...
class CocoKeypoint(data.Dataset):
    def __init__(self,
        ...
        anno_file = f"keypoints_21_{dataset}.json"
        # anno_file = f"person_keypoints_{dataset}{years}.json"     # coco2017

        assert os.path.exists(root), "file '{}' does not exist.".format(root)
        # self.img_root = os.path.join(root, "images")
        self.img_root = os.path.join(root, "images", dataset)
        # self.img_root = os.path.join(root, "images", dataset + years) # coco2017
        ...
        for img_id in img_ids:
            ...
            for ann in anns:
                # only save hand class
                if ann["category_id"] != 0: # person: 1 hand: 0
                    print(f'warning: find not support id: {ann["category_id"]}, only support id: 0 (hand)')
                    continue

                # COCO_val2017_detections_AP_H_56_***.json文件中只有det信息，没有keypoint信息，跳过检查
                ...
                if w > 0 and h > 0:
                    ...
                    # COCO_val2017_detections_AP_H_56_***.json文件中只有det信息，没有keypoint信息，跳过
                    if det_json_path is None:
                    ...
                    self.valid_obj_list.append(info)
                    obj_idx += 1

    def __getitem__(self, idx):
        target = copy.deepcopy(self.valid_obj_list[idx])
    ...
    def __len__(self):
        return len(self.valid_obj_list)
...
if __name__ == '__main__':
    train = CocoKeypoint("/home/dm-wsl/DL/datasets/coco_c430_hand_keypoints", dataset="val")
    print(len(train))
    t = train[0]
    print(t)
