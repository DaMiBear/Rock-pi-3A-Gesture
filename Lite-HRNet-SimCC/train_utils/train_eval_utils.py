...
from .loss import KpLoss, NMTCritierion, KLDiscretLoss


def train_one_epoch(model, optimizer, data_loader, device, epoch,
                    print_freq=50, warmup=False, scaler=None, args=None):
    model.train()
    ...
    # mse = KpLoss()
    if args.coord_representation == 'simcc':
        simcc_loss = NMTCritierion(label_smoothing=0.1)
    elif args.coord_representation == 'sa-simcc':
    ...
    for i, [images, targets] in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast(enabled=scaler is not None):
            ...
            losses = simcc_loss(pred_x, pred_y, targets)
...
@torch.no_grad()
def evaluate(model, data_loader, device, simcc_split_ratio, flip=False, flip_pairs=None):
    if flip:
    ...
    for image, targets in metric_logger.log_every(data_loader, 100, header):
        ...
        pred_x, pred_y = model(images)
        if flip:
            # TODO: change filp code when use SimCC
            pass
        ...
        outputs = transforms.get_final_preds(pred_x, pred_y, simcc_split_ratio, reverse_trans)
