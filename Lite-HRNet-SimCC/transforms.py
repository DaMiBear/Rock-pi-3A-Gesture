...
def flip_images(img):
    assert len(
        img.shape) == 4, 'images has to be [batch_size, channels, height, width]'
...
def flip_back(output_flipped, matched_parts):
    assert len(
        output_flipped.shape) == 4, 'output_flipped has to be [batch_size, num_joints, height, width]'
...
def get_max_preds(batch_heatmaps):
    ...
    """
    assert isinstance(
        batch_heatmaps, torch.Tensor), 'batch_heatmaps should be torch.Tensor'
    ...
    pred_mask = torch.gt(maxvals, 0.0).repeat(
        1, 1, 2).float().to(batch_heatmaps.device)
...
def get_final_preds(batch_pred_x: torch.Tensor,
                    batch_pred_y: torch.Tensor,
                    simcc_split_ratio: float,
                    trans: list = None):
    assert trans is not None
    batch_size = batch_pred_x.size(0)
    num_keypoints = batch_pred_x.size(1)
    # output_x shape:[bs, nkpt, image_w * ratio]
    output_x = torch.nn.functional.softmax(batch_pred_x, dim=2)
    # output_y shape:[bs, nkpt, image_h * ratio]
    output_y = torch.nn.functional.softmax(batch_pred_y, dim=2)
    ...
    output_xy = torch.ones([batch_size, num_keypoints, 2])
    output_xy[:, :, 0] = torch.squeeze(
        torch.true_divide(preds_x, simcc_split_ratio))
    output_xy[:, :, 1] = torch.squeeze(
        torch.true_divide(preds_y, simcc_split_ratio))

    output_xy = output_xy.cpu().numpy()
    # Transform back
    for i in range(batch_size):
        output_xy[i] = affine_points(output_xy[i], trans[i])

    return output_xy, maxvals
...
class AffineTransform(object):
    ...
    def __call__(self, img, target):
        src_xmin, src_ymin, src_xmax, src_ymax = adjust_box(
            *target["box"], fixed_size=self.fixed_size)
        src_w = src_xmax - src_xmin
        src_h = src_ymax - src_ymin
        src_center = np.array(
            [(src_xmin + src_xmax) / 2, (src_ymin + src_ymax) / 2])
        src_p2 = src_center + np.array([0, -src_h / 2])  # top middle
        src_p3 = src_center + np.array([src_w / 2, 0])   # right middle

        dst_center = np.array(
            [(self.fixed_size[1] - 1) / 2, (self.fixed_size[0] - 1) / 2])
        dst_p2 = np.array([(self.fixed_size[1] - 1) / 2, 0])  # top middle
        dst_p3 = np.array(
            [self.fixed_size[1] - 1, (self.fixed_size[0] - 1) / 2])  # right middle
        ...
        if self.rotation is not None:
            ...
            src_p2 = src_center + \
                np.array([src_h / 2 * math.sin(angle), -
                         src_h / 2 * math.cos(angle)])
            src_p3 = src_center + \
                np.array([src_w / 2 * math.cos(angle),
                         src_w / 2 * math.sin(angle)])
...
class KeypointToSimCC(object):
    def __init__(self, coord_representation, simcc_split_ratio, use_different_keypoints_weight, keypoints_weights=None):
        assert coord_representation in ["simcc", 'sa-simcc']
        self.simcc_split_ratio = simcc_split_ratio
        self.use_different_keypoints_weight = use_different_keypoints_weight
        self.keypoints_weights = keypoints_weights
        self.coord_representation = coord_representation
        self.sigma = 2

    def __call__(self, image, target):
        image_size = np.array(image.shape[:2], dtype=np.int64)  # [h, w]
        if self.coord_representation == "simcc":
            keypoints_vis = target["visible"].copy()
            keypoints_split = target["keypoints"].copy()    # shape:[nkpt, 2]
            keypoints_split = np.around(
                keypoints_split * self.simcc_split_ratio).astype(np.int64)  # shape:[nkpt, 2]
            # 数据增强之后，关键点可能会超出图像区域，所以需要过滤
            keypoints_split, keypoints_vis, keypoints_weights = self.filter_target_simdr(keypoints_split,
                                                                          keypoints_vis,
                                                                          image_size * self.simcc_split_ratio)
            target["keypoints"] = torch.as_tensor(
                keypoints_split, dtype=torch.float32)

        elif self.coord_representation == "sa-simcc":
            keypoints_x, keypoints_y, keypoints_weights = self.generate_sa_simdr(
                target["keypoints"], target["visible"], image_size)
            keypoints_x = torch.from_numpy(keypoints_x)
            keypoints_y = torch.from_numpy(keypoints_y)
            keypoints_weights = torch.from_numpy(keypoints_weights)
            target["keypoints_x"] = keypoints_x
            target["keypoints_y"] = keypoints_y

        
        target["kps_weights"] = torch.as_tensor(
            keypoints_weights, dtype=torch.float32)
        return image, target

    def filter_target_simdr(self, keypoints, keypoints_vis, image_size):
        '''
        :param keypoints:  [num_keypoints, 2]
        :param keypoints_vis: [num_keypionts,]
        :param image_size: image_size w, h
        :return: keypoints, keypoints_vis(>0: visible, 0: invisible), keypoints_weights(>0: visible, 0: invisible)
        '''
        num_keypoints = keypoints.shape[0]
        keypoints_weights = np.ones((num_keypoints), dtype=np.float32)
        keypoints_weights[keypoints_vis == 0] = 0     # 保留原来不可见的点
        # keypoints_weights = keypoints_vis.copy()  # 原来是不可见的（为0的），现在仍是不可见的
        ...
        if self.use_different_keypoints_weight:
            keypoints_weights = np.multiply(
                keypoints_weights, self.keypoints_weights)

        return keypoints, keypoints_vis, keypoints_weights

    def generate_sa_simdr(self, keypoints, keypoints_vis, image_size):
        '''
        :param keypoints:  [num_joints, 2]
        :param keypoints_vis: [num_joints, 2]
        image_size: [2] h,w
        :return: target, target_weight(>0: visible, 0: invisible)
        '''
        num_keypoints = keypoints.shape[0]
        keypoints_weights = np.ones((num_keypoints), dtype=np.float32)
        keypoints_weights[keypoints_vis == 0] = 0     # 保留原来不可见的点

        keypoints_x = np.zeros((num_keypoints,
                            int(image_size[1]*self.simcc_split_ratio)),
                            dtype=np.float32)
        keypoints_y = np.zeros((num_keypoints,
                            int(image_size[0]*self.simcc_split_ratio)),
                            dtype=np.float32)

        tmp_size = self.sigma * 3

        for kps_id in range(num_keypoints):
            keypoints_weights[kps_id] = \
                self.adjust_target_weight(
                    keypoints[kps_id], keypoints_weights[kps_id], tmp_size, image_size)
            if keypoints_weights[kps_id] == 0:
                continue

            mu_x = keypoints[kps_id][0] * self.simcc_split_ratio
            mu_y = keypoints[kps_id][1] * self.simcc_split_ratio

            x = np.arange(
                0, int(image_size[1]*self.simcc_split_ratio), 1, np.float32)
            y = np.arange(
                0, int(image_size[0]*self.simcc_split_ratio), 1, np.float32)

            v = keypoints_weights[kps_id]
            if v > 0.5:
                keypoints_x[kps_id] = (
                    np.exp(- ((x - mu_x) ** 2) / (2 * self.sigma ** 2)))/(self.sigma*np.sqrt(np.pi*2))
                keypoints_y[kps_id] = (
                    np.exp(- ((y - mu_y) ** 2) / (2 * self.sigma ** 2)))/(self.sigma*np.sqrt(np.pi*2))
        if self.use_different_keypoints_weight:
            keypoints_weights = np.multiply(keypoints_weights, self.keypoints_weights)

        return keypoints_x, keypoints_y, keypoints_weights

    def adjust_target_weight(self, joint, target_weight, tmp_size, image_size):
        mu_x = joint[0]
        mu_y = joint[1]
...
class KeypointToHeatMap(object):
    def __init__(self,
        ...
        for x in range(kernel_size):
            for y in range(kernel_size):
                kernel[y, x] = np.exp(-((x - x_center) ** 2 +
                                      (y - y_center) ** 2) / (2 * self.sigma ** 2))
        # print(kernel)
    ...
    def __call__(self, image, target):
        ...
        heatmap = np.zeros(
            (num_kps, self.heatmap_hw[0], self.heatmap_hw[1]), dtype=np.float32)
        ...
        for kp_id in range(num_kps):
            ...
            ul = [x - self.kernel_radius, y -
                  self.kernel_radius]  # up-left x,y
            br = [x + self.kernel_radius, y +
                  self.kernel_radius]  # bottom-right x,y
        ...
        target["kps_weights"] = torch.as_tensor(
            kps_weights, dtype=torch.float32)
